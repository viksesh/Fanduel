...
df2 = df[(df["Projected Value"] >= df["Needed Value"])]
print(df2)
...
def combinations():
    ...
    for i in lineups:
        ...
        if 59900 <= values_sum <= 60000: # if all values in dictionary <= 60,000: print
            #print(b)
    ...
    player_per_df = pd.DataFrame.from_dict(player_count, orient = "index")
    ...
    player_count = dict(player_count)
    player_per_df = pd.DataFrame.from_dict(player_count, orient = "index")

    player_per_df.columns = ["Percent"]
